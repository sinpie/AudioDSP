...
- 현재 설정 표는 U7 실제 출력, 요청/유효 profile, fallback, bypass, A/B, Rear mode, convolution 수, 서비스, 장치, format/chunksize를 보여준다. CPU는 `/proc/stat` 표본 차이로 계산한 실제 시스템 사용률을 정수 `%`로 표시하고 첫 표본은 `계산 중`으로 표시한다. load average를 CPU 사용률로 이름 붙이지 않는다.
- FIR 그래프는 처음 열 때 `L/R+Woofer`가 기본이며 toggle로 Woofer를 숨길 수 있다. 브라우저가 WAV를 받아 SVG를 계산하고, 이 그래프는 측정 응답에 곱하는 **보정 전달함수**이며 목표 청취 음압이 아님을 제목과 설명에서 명시한다.
- Light theme의 그래프는 밝은 plot 배경, 진한 축/범례 글자와 충분히 구분되는 L/R/Woofer 곡선을 사용한다. 페이지 배경색을 그대로 그래프 글자색에 재사용해 글자와 배경이 모두 검게 보이는 조합을 허용하지 않는다.
...
- 레벨 검사를 누르는 순간 현재 U7 물리 출력을 session의 `measurement_profile`로 고정한다. 화면은 현재/고정 경로의 일치 여부를 별도 lock 카드로 보여주고, 불일치하면 위치 측정과 A/B를 비활성화한다. A/B 버튼 가까이에 `현재 경로`, `필요 경로`, U7 상단 다이얼을 눌러 바꾸라는 조치를 항상 보이며 물리 경로가 맞으면 약 1.5초 안에 자동 활성화한다. 비활성 버튼의 title만으로 이유를 숨기지 않는다.
...
- 결과 요약은 `공간 통합 계산 · 가중 평균 파워`와 dB 기하평균 대비 L/R/Woofer 중앙 변화량을 함께 보여준다. 이전 response revision이 섞이면 3단계 저장 상태를 amber로 표시하고, `원본 재계산`이 소리를 내지 않는다는 설명과 실제 버튼을 같은 카드에 둔다.
