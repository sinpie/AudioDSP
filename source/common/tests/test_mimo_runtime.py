...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="audiodsp-mimo-runtime-") as temporary_name:
        ...
        mimo_spec = importlib.util.spec_from_file_location("audiodsp_mimo_resource_test", args.measurement.parent / "audiodsp-mimo.py")
        require(mimo_spec is not None and mimo_spec.loader is not None, "MIMO engine import failed")
        mimo = importlib.util.module_from_spec(mimo_spec)
        mimo_spec.loader.exec_module(mimo)
        current_budget = mimo.resource_budget(inputs=2, outputs=4)
        projected_budget = mimo.resource_budget(inputs=6, outputs=7)
        require(current_budget["matrix_paths"] == 8, "2x4 resource path count")
        require(projected_budget["matrix_paths"] == 42, "5.1+dual-sub dense resource path count")
        require(projected_budget["filter_generation_planning_mib"] < 1536, "projected filter generation exceeds 2 GB safe planning limit")
        require(projected_budget["runtime_dsp_planning_mib"] < 512, "projected runtime DSP memory exceeds planning limit")
        ...
        print(json.dumps({
            "result": "PASS", "paths": 8, "taps": 32768,
            "camilladsp_check": True, "backup_schema_2": True, "pi2_block": True,
            "resource_budget_2x4": current_budget,
            "resource_budget_projected_5_1_dual_sub_dense": projected_budget,
        }, indent=2))
    return 0
