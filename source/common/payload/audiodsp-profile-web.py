...
def measurement_panel(job: dict, preview: dict) -> str:
    ...
    if result:
        ...
        peak_cut_reliability = result.get("stereo_peak_cut_reliability") or {}
        cut_policy_current = peak_cut_reliability.get("frequency_dependent_hard_cut_cap") is False
        cut_policy_detail = (
            "숨은 주파수별 상한 없음 · 좁거나 불일치한 피크만 신뢰도 완화"
            if cut_policy_current else "이전 계산 · 새 알고리즘으로 재계산 필요"
        )
