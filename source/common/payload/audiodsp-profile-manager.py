...
def validate_mimo_bank(manifest_path: Path, *, allow_pending_application: bool = False) -> dict[str, Any]:
    try:
    ...
    pending_model_pass = (
        allow_pending_application
        and manifest.get("application_requires_post_filter_measurement") is True
        and isinstance(validation, dict)
        and validation.get("model_pass") is True
        and validation.get("crossover_sum", {}).get("pass") is None
    )
    if not isinstance(validation, dict) or not (validation.get("overall_pass") or pending_model_pass):
...
def preview_mimo(profile: str, manifest_source: Path) -> dict[str, Any]:
    ...
    bank = validate_mimo_bank(manifest_source, allow_pending_application=True)
    settings = load_settings()
    resolved = {
        "requested_profile": settings["requested_profile"],
        "chunksize": max(1024, settings["chunksize"]),
