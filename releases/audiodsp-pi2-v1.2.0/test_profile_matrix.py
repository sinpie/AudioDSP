#!/usr/bin/env python3
"""Exhaustive isolated regression tests for GSonic profiles, Web UI, and U7 HID."""
...
def post_calibration(url: str, orientation: str, path: Path, expected: int = 200) -> bytes:
    boundary = "----GSonicCalibrationBoundary"
    body = (
...
def main() -> int:
    ...
    report: dict[str, Any] = {}
    with tempfile.TemporaryDirectory(prefix="gsonic-profile-matrix-") as temporary_name:
        ...
        environment.update({
            "GSONIC_CONFIG_DIR": str(config),
            "GSONIC_STATE_DIR": str(state),
            "GSONIC_LOCK_PATH": str(root / "manager.lock"),
            "GSONIC_SELECTOR_STATE_PATH": str(root / "u7-selector-state.json"),
            "GSONIC_PREVIEW_STATE_PATH": str(root / "fir-preview.json"),
            "GSONIC_CAMILLADSP": str(args.camilladsp),
            "GSONIC_DISABLE_SERVICE_RESTART": "1",
            "GSONIC_MEASUREMENT": str(args.measurement),
            ...
            "GSONIC_STAGING_DIR": str(staging),
            "GSONIC_PREFERENCES_PATH": str(state / "correction-preferences.json"),
            "GSONIC_AMIXER": str(fake_amixer),
            "GSONIC_U7_MIXER": "hw:U7-test",
        })
        os.environ.update({key: value for key, value in environment.items() if key.startswith("GSONIC_")})
        manager = load_module("gsonic_profile_manager_matrix", args.manager)
        ...
        environment.update({
            "GSONIC_PROFILE_MANAGER": str(args.manager),
            "GSONIC_SHARE_DIR": str(share),
            "GSONIC_APLAY": str(fake_aplay),
            "GSONIC_WEB_HOST": "127.0.0.1",
        })
        with socket.socket() as probe:
            probe.bind(("127.0.0.1", 0))
            port = probe.getsockname()[1]
        environment["GSONIC_WEB_PORT"] = str(port)
