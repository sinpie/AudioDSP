...
- 공통 코드에 MIMO bank/보고서/백업 형식을 포함하되 Pi2의 측정 선택과 실시간 8-path 활성화는 하드 차단
- 모든 SISO 결과에도 FIR 가능·제한·물리 처리·미측정·미인증을 구분한 JSON/Markdown 룸 튜닝 보고서 저장
...
- MIMO 실사용은 SD/백업을 Pi4/5로 이전한 뒤 수행한다. Pi2는 UMIK 녹음과 SISO 보정만 유지한다.
