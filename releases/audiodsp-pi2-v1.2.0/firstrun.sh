...
if [ -x /usr/lib/raspberrypi-sys-mods/imager_custom ]; then
    /usr/lib/raspberrypi-sys-mods/imager_custom set_hostname audiodsp-pi2
else
    old_hostname="$(tr -d ' \t\n\r' < /etc/hostname)"
    echo audiodsp-pi2 > /etc/hostname
    sed -i "s/127.0.1.1.*${old_hostname}/127.0.1.1\taudiodsp-pi2/g" /etc/hosts
fi
...
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJAFs0hAMAw7/9Ueb+2vOQUe7kEPWVcw+WlBckMmODJc audiodsp@audiodsp-pi2
KEYEOF
...
/usr/local/bin/audiodsp-profile-manager.py set-chunksize 2048 --no-restart >/var/log/audiodsp-chunksize-initial.json
