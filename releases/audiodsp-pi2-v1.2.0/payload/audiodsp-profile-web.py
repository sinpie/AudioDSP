...
def measurement_panel(job: dict, preview: dict) -> str:
    ...
    position_count_note = (
        '<p class="form-note position-count-note"><b>Fast 1위치:</b> 기준점 최적화 · 빠름 · 위치 이동 시 과보정 가능<br>'
        '<b>Standard 3위치:</b> 중앙+좌우 공통 문제 보정 · 저역/crossover/좌석 안정성 권장</p>'
    )
    ...
    if state == "idle":
        ...
        pre_validation_control = ""
        if job.get("mode") == "lrw" and positions == total and not job.get("validation"):
            pre_validation_control = f'''<form method="post" action="/measurement/validation" onsubmit="return confirm('마이크를 위치 1 기준점으로 되돌리세요. FIR을 우회한 물리 L+Woofer/R+Woofer 진단음을 재생합니다. 시작할까요?')"><button{disabled}>선택 진단 · FIR 전 기준점 L+Woofer / R+Woofer</button></form>'''
        controls = f"""
        {session_settings}
        {session_library}
        <div class="measure-actions" data-measurement-step-content="2">
          <form method="post" action="/measurement/level" id="measurement-step-2" onsubmit="return confirm('현재 U7 물리 출력 경로를 이 측정에 고정하고 레벨 검사를 실행합니다. 기존 {total}위치 측정과 FIR 결과는 초기화됩니다. 계속할까요?')"><button{disabled}>현재 U7 경로 고정 · 5초 무음 + 5초 백색소음 · {noise_level_dbfs} dBFS</button></form>
        ...
        <p class="muted" data-measurement-step-content="3"><b>빠른 측정:</b> 위치 1은 Front L → Woofer → Front R 순서입니다. 한 위치의 sweep을 DSP 재시작이나 FFT 대기 없이 먼저 연속 녹음하고, 소리가 모두 멈춘 뒤 응답을 일괄 계산합니다. 완료된 채널은 재시도할 때 다시 재생하지 않습니다.</p><p class="form-note" data-measurement-step-content="3"><b>선택 진단의 범위:</b> 마이크를 위치 1 기준점으로 되돌려 FIR 적용 전 물리 합산·극성 문제를 참고용으로 확인합니다. 4단계 FIR을 통과한 Target 달성 판정은 5단계의 ‘Preview FIR 적용 후 합산 실측’에서 별도로 수행하며, 이 실행은 3단계 원측정이나 4단계 결과를 초기화하지 않습니다.</p>"""
        if positions == total:
            ...
            if mode in ("lrw", "mimo_one_sub", "mimo_dual_sub"):
                ...
                crossover_note = f'<p class="form-note"><b>기본 ON:</b> Front에는 LR4 HPF, Woofer에는 LR4 LPF를 넣고 선택한 {total}위치의 L+Woofer/R+Woofer 합산 상한을 cut-only로 보호합니다. 모두 32768탭 WAV 안에 합쳐지므로 CamillaDSP filter stage나 block latency는 늘지 않습니다.</p>'
            else:
    ...
    if result:
        ...
        independent_failed = isinstance(self_validation.get("independent_positions"), dict) and not bool(self_validation["independent_positions"].get("pass"))
        validation_failed = (core_failed or required_fit_failed or independent_failed or crossover_failed) and not stale_result
        validation_rows = []
        ...
        if crossover_check.get("required"):
            ...
            if crossover_check.get("pass") is None:
                add_validation_row("Front+Woofer crossover 합산", "na", "검증 대기 · 먼저 이번 튜닝 Preview를 적용한 뒤 아래의 FIR 적용 후 합산 측정을 실행하세요.")
            else:
                crossover_guide = "실측 그래프에서 높으면 Woofer trim/억제를 낮추고, crossover 딥이면 극성·거리·주파수를 조정한 뒤 FIR을 다시 계산하세요. 원측정을 다시 할 필요는 없습니다."
        ...
        if stale_result:
            ...
            stale_result_notice = '<div class="diagnostic-note" role="status"><b>FIR 적용 후 합산 실측 대기</b><br>WAV 다운로드와 A/B 확인은 가능합니다. 이번 튜닝 Preview를 적용한 뒤 아래에서 선택한 1/3위치의 L+Woofer/R+Woofer 검증을 완료하면 정식 적용 여부를 판정합니다.</div>'
            apply_actions = '<button disabled>사후 합산 실측 후 정식 적용 가능</button>'
        ...
        if job.get("mode") == "lrw" and crossover.get("enabled"):
