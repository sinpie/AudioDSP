...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- 전체 검증: {'PASS' if result['self_validation']['overall_pass'] else 'FAIL'}",
        "",
        "## 예측 개선",
        "",
    ]
...
def build(session_path: Path, output_dir: Path, options: dict[str, Any], engine_path: Path | None = None, *, allow_unsupported: bool = False) -> dict[str, Any]:
    ...
    for channel, channel_name in enumerate(("left", "right")):
        before_errors, after_errors, before_std, after_std = [], [], [], []
        before_curve, after_curve, target_curve_values = [], [], []
        ...
        for frequency in graph_frequencies:
            ...
            before_curve.append(statistics.median(before_levels))
            after_curve.append(statistics.median(after_levels))
            if 20.0 <= frequency <= high_hz:
                before_errors.extend(abs(value - target_db_value) for value in before_levels)
                after_errors.extend(abs(value - target_db_value) for value in after_levels)
